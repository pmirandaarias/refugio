...
class SolicitudUpdate(UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        persona = self.second_model.objects.get(id=solicitud.persona_id)
        form = self.form_class(request.POST, instance=solicitud) # sino colocamos instance va a crear un nuevo objeto en vez de actualizar
        form2 = self.second_form_class(request.POST, instance=persona) # sino colocamos instance va a crear un nuevo objeto en vez de actualizar
        if form.is_valid() and form2.is_valid():
            form.save()
            form2.save()
            return HttpResponseRedirect(self.get_success_url())
